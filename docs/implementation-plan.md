...
3. **Day-type & schedule + resolver** — CRUD for `dayType`/`weeklySchedule`/`dateOverride`; `resolveDayType`/`fastingState`; Today's eating-window card wired live. *Verify*: exhaustive resolver/fasting-state Jest suite (override precedence, midnight-crossing, DST-pinned instants); manual live-update check. — **DONE** (manual live-update check still needs a human on-device/simulator)

   > **Addendum:** two design refinements beyond the original sketch, both confirmed necessary against the spec:
   > - **A third "flexible" day kind.** The spec's own Day-Type table marks both Saturday (Travel/Wildcard) and Sunday (Game Day) window as "Flexible" — not a hard fast, not a timed window. `computeFastingState`/`isWithinEatingWindow` in `src/domain/fasting/fastingState.ts` classify every day as `fasting` (isFastDay) / `flexible` (no window, not fasting — always "eating", no countdown) / `timed`, rather than collapsing "no window configured" into a single case.
   > - **`computeFastingState` takes `(now, yesterday, today, tomorrow)`, not just `(now, today, tomorrow)`.** A midnight-crossing window opened the previous evening (e.g. 20:00–04:00) must still register as "eating" in the early hours of today even when today's own DayType is unrelated (a hard fast, a different window, flexible). The function checks yesterday's resolved window first, generically, before falling back to today's own state — this is what makes edge case #1 (windows crossing midnight) actually correct rather than just "doesn't crash."
   >
   > Also: `src/lib/datetime.ts`'s `anchorTimeToDate`/`todayIsoInZone` were relocated to `src/domain/datetime.ts` — they're exactly the primitives the resolver/fasting-state math depends on, so per the domain/lib separation rule they belong in `domain`, not `lib` (which now holds only the device-facing `deviceTimezone()`). And the Phase 0 smoke test (a bare RTL render of the Today screen) was retired rather than patched: the Today screen now pulls live data through `useFastingState`/`useUserProfile`, which import `src/db/client.ts` and crash under Jest without a real or mocked `expo-sqlite` native binding. Building that mock now would mean guessing at a shape before Phase 8 (which explicitly needs an RTL test of the Travel screen) defines the real requirement — deferred to that phase instead of speculatively solved here. Phase 2 verification instead relied on the domain Jest suite plus a real `expo export --platform web` bundle build (which exercises the full DB/migration/resolver wiring end-to-end, including the brand-new-profile empty state) — see the Verification section below.
4. **Habit stack** — `routineStep`/`routineCompletion` repos + editor; `nextStep.ts`; checklist + next-action banner on Today. *Verify*: Jest for ordering/tie-breaks/all-done; manual per-date isolation check. — **DONE** (manual per-date isolation check still needs a human on-device/simulator)
5. **Meal presets & food logging** — preset/stack CRUD; `foodLogRepo.logFood` with snapshotting; `onceGuard` for duplicate taps; outside-window flagging; Today's meal grid + macro progress. *Verify*: Jest for macros + snapshot-independence + onceGuard; manual edit-preset-after-logging check. — **DONE** (manual edit-preset-after-logging check still needs a human on-device/simulator, though the same scenario is now also covered by an automated integration test — see addendum)

   > **Addendum:** the snapshot-independence and off-window-logging edge cases are fundamentally database-round-trip properties (write via a preset, mutate the preset, re-read the log), not pure-function ones — a bare `computeMacroProgress` unit test can't actually exercise them. Rather than deferring this to Phase 8 as originally planned for DB-touching tests, built a proper Node-based integration-test harness now: `src/db/testClient.ts` runs the real schema and real committed migrations against an in-memory `better-sqlite3` database (a pure Node native module, no Expo runtime needed), wired in via `jest.config.js`'s `moduleNameMapper` (redirecting `@/src/db/client` for the `node` project only — this is also why every repository now imports the client via that alias instead of a relative path, for a stable, mappable specifier). `foodLogRepo.test.ts` exercises the real snapshot-independence guarantee end to end. This same harness will carry Phase 6's mid-week-target-change test and any future repo-level test. Phase 8's RTL/component-mock question is unaffected — that's about rendering screens in React Native Testing Library, a separate concern from this Node-only repository testing.
   >
   > Also: `computeFastingState`'s `isWithinEatingWindow` only ever checked a single resolved day; a new `isWithinAnyEatingWindow(at, yesterday, today)` in `fastingState.ts` checks yesterday's window too (mirroring `computeFastingState`'s own yesterday-first logic), so `foodLogRepo.logFood` correctly flags a 2am log as within-window when it falls inside a midnight-crossing window opened the previous evening.
6. **Target snapshot + weekly budget** — `dailyLogSnapshot` + `ensureDailySnapshot` wired into every write repo; `weeklyBudget.ts`; History reads snapshot-first. *Verify*: the mid-week-target-change scenario end-to-end, both in Jest and manually. — **DONE** (manual walkthrough still needs a human on-device/simulator)

   > **Addendum:** "wired into every write repo" currently means `foodLogRepo.logFood` and `routineCompletionRepo.setRoutineStepStatus` (the two write paths that exist so far) — `setRoutineStepStatus` now takes a `resolvedToday: ResolvedDayType` fourth argument for this, threaded through `useTodayRoutine` (which now takes a single `ResolvedDayType` instead of separate `dayTypeId`/`date` params). Phase 7's hydration/activity/weight repos will call `ensureDailySnapshot` the same way from day one. "History reads snapshot-first" is implemented as `useWeeklyBudget` (Today tab's new "This week" card): snapshot-first per date, live `resolveDayType()` fallback for untouched dates — a full History tab for browsing/editing arbitrary past days is still Phase 9's job. Verified via a real integration test in `dailyLogSnapshotRepo.test.ts` using the `better-sqlite3` harness from Phase 5: log Monday, change the DayType's targets, assert Monday's snapshot is frozen at the old values while untouched Tuesday has no snapshot row at all (and picks up new values once it does get one).
7. **Hydration** — `hydrationLog` repo + quick-add config; `hydration.ts` (no clamping). *Verify*: Jest for over-goal math; manual over-goal display check. — **DONE** (manual over-goal display check still needs a human on-device/simulator)

   > **Addendum:** quick-add amounts (8/16/24 oz) are fixed UI constants, not a Settings-editable list — the spec's "configuration over hard-coding" law is about not hard-coding the *user's personal targets* (weight, calories, windows, the hydration *goal* itself, which is already profile data), not about every UI convenience value; the spec's own enumerated Settings/Plan screen list doesn't call out quick-add amounts. `hydrationLogRepo.logHydration` follows the same pattern as `foodLogRepo.logFood`: takes the resolved day type, calls `ensureDailySnapshot`, stores the value unclamped.
8. **Travel/Wildcard + discovery adapter** — override-based day-type switch; provider interface + mock + TanStack Query hooks; Travel screen with always-available manual fallback. *Verify*: RTL test forcing provider rejection; manual override-doesn't-corrupt-schedule check. — **DONE** (manual override-doesn't-corrupt-schedule check still needs a human on-device/simulator, though the resolver-level version of that guarantee already has a Phase 3 unit test)

   > **Addendum:** this phase is exactly where the Phase 2/4 "defer DB-mocking for RTL until a concrete need defines the shape" bet paid off — `app/(tabs)/__tests__/travel.test.tsx` (mocking `@/src/services/location-discovery` to reject, then driving the manual form) is that concrete need. Getting it running surfaced two real gotchas worth remembering: (1) `@testing-library/react-native` v14's `render`, `fireEvent`, and `act` are *all* async now — an un-awaited `fireEvent.press` silently fails to flush the state update it triggers, which looks exactly like "the write didn't happen" rather than an obvious error; (2) `drizzle-orm`'s `useLiveQuery` imports `addDatabaseChangeListener` directly from `expo-sqlite`, independent of which `db` instance a query targets — swapping `@/src/db/client` alone (the Phase 4 approach) isn't enough for component tests, hence `__mocks__/expo-sqlite.js` (a no-op subscription stub — see the Testing section in README.md for what this does and doesn't cover) and `--forceExit` on the `test` script (TanStack Query's GC timers otherwise keep the process alive).
   >
   > Location itself is a fixed placeholder (`{ latitude: 0, longitude: 0 }`) rather than real GPS via `expo-location` — out of this phase's scope per the spec's "MVP can run with mocked/local data" allowance; a real location integration is a natural post-MVP addition alongside a real discovery provider.
9. **Weight log, polish, history editing, empty states, docs hardening** — `weightLog` + `weightChange.ts`; History edit/delete with live recalculation; empty states for fresh profile and zero-log dates; fill remaining edge-case tests; finalize `CLAUDE.md`/`README.md`. *Verify*: full `npm test` green, `tsc --noEmit` clean, full manual walkthrough from fresh install. — **DONE** (the full manual fresh-install walkthrough still needs a human on-device/simulator)

   > **Addendum:** History browsing is a 14-day rolling list (`useHistoryDays`) plus a day-detail view (`useDayDetail`), not an unbounded calendar — sufficient for reviewing/correcting recent entries, which is what FR-10 actually asks for. "Edit" is implemented as delete-then-relog rather than in-place field editing, since the FoodLog/HydrationLog snapshot design (Phase 4/6) makes "edit" and "delete + create a new snapshot" nearly the same operation anyway, and delete-then-relog is far less code for the same outcome. The zero-log-date empty state got a real component test (`app/(tabs)/history/__tests__/date.test.tsx`) using the same DB-integration-test pattern as Travel — writing it surfaced a genuine bug in `testClient.ts`'s `resetTestDb()`: it deleted tables in `sqlite_master` order, which isn't dependency-safe (a `dailyLogSnapshot` row can still reference a `dayType` row when the clear runs), so it now disables FK enforcement for the clear only. This bug had been latent since Phase 4 — every earlier test file happened to avoid the exact cross-table combination that triggers it.
   >
   > Also added, beyond the original phase description, because the spec's own "Documentation" acceptance criterion explicitly requires the README to cover "seed/reset behavior": **Settings > Data > Reset all data** (`src/db/reset.ts`), a real (not test-only) irreversible wipe of every table, exercised via the same reactive `useHasProfile` mechanism that gates onboarding — no explicit navigation needed after a reset, the app just redirects itself. This is the "reset" half of the plan's original "Seed / Demo Data Strategy" section; the "seed" half (a one-tap "load a demo profile" populating all the spec's Section-2 example data) was **not** built — it's a real, deliberate scope cut given everything else in this phase, not an oversight. Onboarding still only supports entering a real profile by hand.
   >
   > **Other known gaps carried past this MVP pass, for the same reason (explicit scope cut, not oversight):** no `activityLog`/exercise-session logging (the table exists in the schema from Phase 1; workout *completion* is tracked via the habit-stack routine step, but duration/notes/distance from spec Section 5 aren't captured); no real GPS integration for Travel (Phase 8's fixed placeholder location stands); no push notifications/reminders (the fasting countdown has been foreground-only since Phase 2, as flagged in that phase's assumptions).

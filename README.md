...
**Feature expansion in progress** (`docs/feature-expansion-plan.md`, Phases 10–20): supplements, a virtual pantry with overridable nutrition data, meal planning, and a real backend for accountability-partner sharing.
...
- **Phase 17 (accountability — backend skeleton)** — done: a `backend/` service (Fastify + Drizzle/Postgres + Zod, its own Vitest suite) with lazy account creation, a short-lived-code primitive shared by multi-device linking and partner pairing, and a Settings > Accountability screen to create an account, generate/accept invite codes, and see both pairing directions. Deployed to Render (`goalventure-backend`) — see `docs/feature-expansion-plan.md`'s Phase 17 addendum for its current one-step deploy blocker (linking `DATABASE_URL`).
- **Phase 18 (accountability — configurable check-ins + summary push)** — done: `check_in_item` CRUD (an account's own configured fields, e.g. "100oz water" — never a hardcoded list) with in-app screens to manage items and send today's check-in, and a `daily_summary` upsert/read endpoint with partner reads authorized against a confirmed `partnership` row.
- **Phase 19 (accountability — partner web view)** — done: a plain HTML/vanilla-JS page served directly from the backend (`backend/public/`, no build step, no separate deploy) lets a partner create a lightweight account, accept an invite code, and glance at the last 14 days of someone's check-ins with real labels — no mobile app install required.
- **Phase 20 (UAT gap-fixes)** — done: closed real gaps a first UAT pass found — `alcoholRule` now actually settable, new `healthFocus`/`palateNotes` profile fields, a post-onboarding Settings > Profile editor, and the Phase 17/18 mobile UI above. See `docs/feature-expansion-plan.md`'s Phase 20 addendum for the full list, including two things evaluated and deliberately *not* built (range-based targets, a supplement-dosing schema change).
